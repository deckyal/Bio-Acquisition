...
def timeStampToNP64(dateData):
    #first change to local time
    t = time.localtime(dateData)
    ts = datetime.datetime.fromtimestamp(time.mktime(t))
    
    #ts = datetime.datetime.utcfromtimestamp(dateData)
...
class visImageThread(QThread):
    def collectProcessData(self):
        ...
        if theIndex < len(toDisplayImgData):
            ...
            try:
                ...
                labelTimeImage.setText(str(toDisplayImgTime[theIndex])+'--To--'+self.endTime)
    ...
    def __init__(self, *args, **kwargs):
        ...
        global displayFPS,toDisplayImgTime
        self.endTime = str(toDisplayImgTime[-1])
        self.fps = displayFPS
...
class visBioThread(QThread):
    def collectProcessData(self):
        ...
        #print('step bio : ',stepBio)
        if theIndex < len(toDisplayBioData) and self.toUpdate:
            self.runningIndexBio += 1
            global gvECG,gvEDA,displayRow
            #print(self.runningIndexBio)
            
            bio = toDisplayBioData[theIndex]#[toDisplayBioData[0][theIndex], toDisplayBioData[1][theIndex]]
            
            #print('cuk',toDisplayBioData[0:10])
            
            labelTimeBio.setText(str(toDisplayBioTime[theIndex])+'--To--'+self.endTime)
            ...
            self.visBio.append([self.runningCount, bio])  # i,time,ecg,eda
            vData = np.asarray(self.visBio)
            #print(self.visBio)
            
            #print('thedata',len(vData),vData[:,1].shape)

            x = vData[:, 0]  # [runningCount]#random.normal(size = 10)
            bion = np.stack(vData[:,1])
            #print(bion.shape)
    ...
    def __init__(self, *args, **kwargs):
        ...
        global displayDPS
        global toDisplayBioTime
        self.endTime = str(toDisplayBioTime[-1])
        self.fps = displayDPS
        self.dataCollectionTimer =QtCore.QTimer()
        self.dataCollectionTimer.moveToThread(self)
        self.dataCollectionTimer.timeout.connect(self.collectProcessData)
...
class CameraQThread(QThread):
    def collectProcessData(self):
        ...
        global dirName
        global camera
        global enabledCameraG
        
        if not enabledCameraG :
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.listWidgetView_r.clicked.connect(self.redrawGraph)
        ...
        self.radioButtonA.toggled.connect(self.updateShowAll)
        self.radioButtonP.toggled.connect(self.updateShowPartial)
        
        self.radioButtonE_y.toggled.connect(self.updateShowExtYes)
        self.radioButtonE_n.toggled.connect(self.updateShowExtNo)
        
        self.radioButtonCam_y.toggled.connect(self.updateShowCamYes)
        self.radioButtonCam_n.toggled.connect(self.updateShowCamNo)
        ...
        self.spinBoxBSRSliceMin.valueChanged.connect(self.updateSliceEvery)
        self.listWidgetView.clicked.connect(self.updateToDisplay)
        
        
        #metadata
        self.dirFolder = "C:\\Users\\cmtech\Documents\\PythonScripts\\"
        self.dirFolderE = 'D:\\ECG-DATA\\'#"C:\\Users\\cmtech\Documents\\Out\\"#!!!
        ...
        initializeCamera = self.readCameraSetting()
        
        if initializeCamera : 
            #try to initialize camera, if failed revert the setting
            try : 
                global camera;
                camera = CameraThread(self)
            except :
                QMessageBox.information(self, "No Camera is detected", "Plug the camera to capture the image, for now it will be disabled to continue. It can be re-activated on Enable-Camera option") 
                initializeCamera = False
                self.writeCameraSetting(0)
        ...
        ntry = 3
        connected = False
        while not connected and tryingNumber < ntry : 
            try : 
                Bio = BioThread(self)
                connected = True
            except : 
                QMessageBox.information(self, "No Bio-Signal device is detected (retry "+str(tryingNumber)+" of "+str(ntry)+")","Activate the machine and ensure stable flashing of light. If not, try to restart the machine by holding the on-off button for 10 seconds")
            tryingNumber+=1
        ...
        if initializeCamera : 
            camera.startThread()
            self.cameraIsUsed = True
            enabledCameraG = True
            self.radioButtonCam_y.setChecked(True)
        else : 
            self.cameraIsUsed = False    
            enabledCameraG = False
            self.radioButtonCam_y.setChecked(False)
        ...
        #To display all data in the replaying
        ...
        self.enabledExternalDisk = False
        self.enabledCamera = True
    ...
    def readCameraSetting(self):
        fileName = self.dirFolder+"cameraSetting.txt"
        return (int([line.rstrip() for line in open(fileName,'r')][0])==1)
    
    def writeCameraSetting(self,value):
        fileName = self.dirFolder+"cameraSetting.txt"
    ...
    def updateShowAll(self): 
        self.displayAll = True
        
    def updateShowPartial(self): 
        self.displayAll = False
        
    def updateShowExtYes(self):
        #check if external disk is exist, otherwise left
        if os.path.isdir(self.dirFolderE) :
            self.enabledExternalDisk = True 
            self.reloadForm()
        else : 
            QMessageBox.information(self, "No External", "No External SSD is detected")
            self.radioButtonE_n.setChecked(True)
        
    def updateShowExtNo(self):
        self.enabledExternalDisk = False
        self.reloadForm()
    
    def updateShowCamYes(self):
        global enabledCameraG
        enabledCameraG = True
    ...
    def updateShowCamNo(self):
        global enabledCameraG
        enabledCameraG = False
        
        self.enabledCamera = False
        self.reloadCamera()
    ...
    def updateSliceEvery(self):
        global sliceEvery
        sliceEvery = int(self.spinBoxBSRSliceMin.text())
    ...
    def transferToIn(self):
        ...
        #npy
        for x in npyFile : 
            os.rename(x, newFolder+self.getFileName(x))
    ...
    def redrawGraph(self):
        selRow = self.listWidgetView_r.currentRow()
        self.graphSingle(selRow)
    ...
    def getListInfo(self):
        ...
        date = datetime.datetime.strptime(str(row[1]), '%Y-%m-%d %H:%M:%S.%f')
        date2 = datetime.datetime.strptime(str(row[-1]), '%Y-%m-%d %H:%M:%S.%f')
        
        self.labelTimeImage.setText(str(date)+"---"+str(date2))
        print('teh data mf',str(date))
        ...
        print("meta data ",self.metaData[selRow])
    ...
    def doReplay(self):
        ...
        #
        '''if not self.displayAll and False: 
            print('updating')
            data = self.curData
            
            for x in range(1,len(self.listToRead)) :
                temp = np.load(str(self.listToRead[x]))
            ...
            self.tempBio =[datetimeR,ecg,eda]'''
        #
        ...
        with open(self.csvTemp) as csvfile:
            ...
            for row in csvReader:
                ...
                if not self.displayAll : 
                    if np.datetime64(dateEnd) > np.datetime64(self.tempBio[0][-1]):
        ...
        print(time1,'-',time2,'length : ',len(self.tempBio[0]))
        print('DPS was ',DPS)
        ...
        print('istart1-',istart,listTimes[istart[0]],self.tempBio[0][istart[1]])
        print('iend1-',iend,listTimes[iend[0]],self.tempBio[0][iend[1]])

        #self.toDisplayBioData = np.stack([self.tempBio[1][istart[1]:iend[1]],self.tempBio[2][istart[1]:iend[1]]])
        #print(self.tempBio[1],len(self.tempBio[1]),istart[1],iend[1])
        self.toDisplayBioData = self.tempBio[1][istart[1]:iend[1]]
        
        print(len(self.toDisplayBioData))
        print(self.toDisplayBioData[0].shape)
        ...
        print(self.toDisplayBioTime,len(self.toDisplayBioTime))
        ...
        DPS = truediv(len(self.toDisplayBioData),(self.toDisplayBioTime[-1] - self.toDisplayBioTime[0]).item().total_seconds())
        
        print('deno',len(self.toDisplayBioData[0]))
        print('divider = ',(self.toDisplayBioTime[-1] - self.toDisplayBioTime[0],(self.toDisplayBioTime[-1] - self.toDisplayBioTime[0]).item().total_seconds()))
        #DPS = truediv(len(self.tempBio[0]),(time2 - time1).item().total_seconds())
        ...
        stepImage = int(math.ceil(truediv(FPS,displayFPS)))
    ...
    def adjustPlayback(self):
        ...
        if self.isViewing :
            self.isViewing = False
            self.listWidgetView_r.setEnabled(False)
    ...
    def setButton(self,isRecording):
        self.groupBox_EC.setEnabled(not isRecording)
    ...
    def reloadMachine(self):
        self.turnOffMachine()
        self.turnOnMachine()
    
    def reloadForm(self) :
        self.updateList()
        if self.enabledExternalDisk : 
            self.updateListE()
    ...
    def updateList(self):
        ...
        if len(listFolder) <=0:
            ...
            print('all meta data',self.metaData)
    ...
    def graphBio(self,filedata):
        ...
        data = np.load(str(filedata[0]))
        
        #concatenate other as one list
        if self.displayAll :
            for x in range(1,len(filedata)) :
                temp = np.load(str(filedata[x]))
                data = np.concatenate((data,temp), axis=0)
                #print(temp.shape,data.shape)
        else : 
            self.listToRead = filedata
            self.curData = data
                 
        
        #data = data[::4]
        #self.tempBio =data
        print("The data shape : ",'#'*8,data.shape)

        dtt = data[:, -1]
        
        fv = np.vectorize(timeStampToNP64)
        datetime = fv(dtt)
        
        labelTimeBio.setText(str(datetime[0])+'---'+str(datetime[-1]))

        bio = data

        #print(bio[0:10])
        self.tempBio =[datetime,bio]
        self.graphSingle(0)
        '''
        datetime = data[:, 1]
        data = data[:, 0]
        ...
        self.listWidgetView_r.setCurrentRow(0)
        self.listWidgetView_r.setEnabled(True)

    
    
    def graphSingle(self,sel):
        datetime = self.tempBio[0]
        toDisplay = self.tempBio[1][:,sel]
        
        
        print(toDisplay)
        dt = datetime[-1] - datetime[0]
        seconds = dt.item().total_seconds()
        time = np.linspace(0, seconds, len(toDisplay))
        
        self.ax.clear()
        self.ax.plot(time, toDisplay, '-')
        self.canvas.draw()
    ...
    def reloadCamera(self) :
        ...
        if self.enabledCamera : 
            if self.cameraIsUsed: 
                return
            else :
                global camera;
                try : 
                    ...
                    print('no camera!')
                    #revert back 
                    QMessageBox.information(self, "No Camera is detected", "Please ensure the camera is connected to capture the images")
                    self.radioButtonCam_n.setChecked(True)
                    return
    ...
    def updateVis(self):
        #print('updating')
        if not self.cameraIsUsed : 
            img = np.zeros([10,10,3])#cv2.imread(self.dirFolder+"noimage.png")
            cameraTime = str("Camera is Detached")
            self.labelTimeImage.setText("C-"+cameraTime)
        else : 
            data = camera.temp
            try:
                img = data[0]
                cameraTime = str(data[1])
                self.labelTimeImage.setText(cameraTime)
            except : 
                img = np.zeros([10,10,3])#cv2.imread(self.dirFolder+"noimage.png")
                cameraTime = "Camera is initializing"
                self.labelTimeImage.setText(cameraTime)
        ...
        self.labelTimeBio.setText(str(timeStampToNP64(bio[-1])))#str(bio[1]))
        
        if self.cameraIsUsed : 
            try : 
                self.labelSpeed.setText('FPS:'+str(round(camera.fps))+' - DPS:'+str(round(Bio.fps)))
            except : 
                self.labelSpeed.setText('FPS: Init - DPS:'+str(round(Bio.fps)))
        else :
            self.labelSpeed.setText('FPS: NONE - DPS:'+str(round(Bio.fps)))
            
        if self.isRecording :
            self.labelInfo.setText(
                'Now Recording...,' +str(datetime.timedelta(seconds=time.time()-self.startTimeR)) + ' Seconds Elapsed')
        #self.updateGraph()
    ...
    def updateGraph(self):
        global visRunningCount,runningCount,visBio,numThread
        runningCount+=1
        visRunningCount+=1
        ...
        global displayEvery,displayRow
        ...
        visBio.append([runningCount, bio])  # i,time,ecg,eda
        vData = np.stack(visBio)
        
        x = vData[:,0]  # [runningCount]#random.normal(size = 10)
        bioData = np.stack(vData[:,1])
        '''yECG = bioData[:,0]
        yEDA = bioData[:,1]'''
    ...
    def showlcd(self):
        ...
        self.updateUsedDisk() 
        self.labelTC.setText(str(numThread))

    def startRecordAllModality(self):
        fileName = self.dirFolder+"setting.txt"
        
        self.j = int([line.rstrip() for line in open(fileName,'r')][0])
        j = self.j
        ...
        self.bioNameShorten = directory + '/' + 'bio_' + str(j)
    ...
    def stopRecordBio(self):
        ...
        #t = Thread(target=self.saveBioToNpy(), args=()).start() #updated here
        self.saveBioToNpy()
    ...
    def saveBioToNpy(self):
        global listBios,recordingDPS,sliceEvery
        import math 
        limit_record = recordingDPS * (60*sliceEvery) #split every five minutes
        
        data = np.asarray(np.stack(listBios))
        split = truediv(len(listBios),limit_record)
        print(data)
        print(split)
        
        nSplit = int(math.ceil(split)) #100/2 = 5
        print('nsplit',len(listBios),limit_record,nSplit)
        for i in range(nSplit): 
            print('saving ',i)
            iStart = i*limit_record
            
            if i < nSplit : 
                iEnd = iStart+limit_record
            else : 
                iEnd = len(listBios)
                
            toSave = data[iStart:iEnd]
            print(toSave)
            toSave = toSave.copy()
            Thread(target=saveSingleNPY, args=(self.bioNameShorten+'-'+str(i)+'.npy',toSave)).start()
            
            #np.save(self.bioNameShorten+'-'+str(i)+'.npy', data[iStart:iEnd])
